...
protected:
    ...
    static FILE *createRelativePath( FILE *absolute );
...
public:
    ...
    static std::vector<std::string> splitOptions( const std::string &quotedOptionString ) throw( std::exception );
...
//ORIGINAL LINE: @SuppressWarnings("unchecked") public static <T> T cast(Object x)
    ...
    /// <returns> the indicated option or an empty string </returns>
    /// <exception cref="Exception"> if the option indicated by the flag can't be found </exception>
    static std::string getOption( const std::string &flag, std::vector<std::string> &options ) throw( std::exception );
