...
namespace weka {
    namespace classifiers {
        namespace trees {
            namespace j48 {
                using weka::core::Capabilities;
                using weka::core::CapabilitiesHandler;
                using weka::core::Drawable;
                using weka::core::Instance;
                using weka::core::Instances;
                using weka::core::RevisionHandler;
...
long long ClassifierTree::PRINTED_NODES = 0;
                ...
                Capabilities *ClassifierTree::getCapabilities() {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                  return text->toString() + std::string(L"}\n");
                }
                ...
                std::string ClassifierTree::prefix() throw(std::exception) {
                  ...
                  if( misLeaf ) {
                    text->append( std::string( L"[" ) + mlocalModel->dumpLabel( 0, mtrain ) + std::string( L"]" ) );
                  } else {
                    prefixTree( text );
                  }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                ...
                std::vector<StringBuffer*> ClassifierTree::toSource( const std::string &className ) throw( std::exception ) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                ...
                std::string ClassifierTree::getRevision() {
                  return RevisionUtils::extract( L"$Revision$" );
                }
            }
        }
    }
